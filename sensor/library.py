...
def connect_serial_session(comName):
	...
	try:
		...
		ser = serial.Serial(comName, baudrate=115200, timeout=3)
...
def enroll_finger_to_device(session, library):
	...
	if i == library.OK:
		...
		if i == library.BADLOCATION:
			print("Bad storage location")
		elif i == library.FLASHERR:
...
def search_fingerprint_on_device(session, library):
	"""Get a finger print image, template it, and see if it matches!"""
	print("Waiting for image...")
	while session.get_image() != library.OK:
		pass
	print("Templating...")
	if session.image_2_tz(1) != library.OK:
		return False
	print("Searching...")
	if session.finger_search() != library.OK:
		return False
	return True

def enroll_save_to_file(session, library, save_location, file_name):
...
def fingerprint_check_one_file(session, library, save_location, file_name):
	...
	if i == library.OK:
		...
		return True
	if i == library.NOMATCH:
		print("Templates do not match!")
	else:
		print("Other error!")
	return False

def fingerprint_check_all_file(session, library, save_location):
	"""Compares a new fingerprint template to an existing template stored in a file
	This is useful when templates are stored centrally (i.e. in a database)"""
